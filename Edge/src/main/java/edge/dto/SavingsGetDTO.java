...
public class SavingsGetDTO {
    ...
    public SavingsGetDTO(Long id, @NotNull(message = "A balance is required") Money balance, Date dateOfCreation, User user, List<TransactionDTO> receivedTransactions, List<TransactionDTO> sentTransactions, @NotNull(message = "A secretKey is required") String secretKey, Status status, @DecimalMax(value = "1000", message = "The minimumBalance must be less than 1000") @DecimalMin(value = "100", message = "The minimumBalance must be greater then 100") BigDecimal minimumBalance, @DecimalMax(value = "0.5", message = "The interestRate can not be greater than 0.5") @Positive(message = "The interestRate must be positive") BigDecimal interestRate, Date lastInterestDate) {
        this.id = id;
